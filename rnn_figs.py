...
def load_data():
    return pickle.load(open("./rnn_util/rnn_data.p", "rb"))
...
def get_min_error(error, itrial=-1):
    ...
    error_sum = np.nanmedian(error_itrial, axis=-1)

    # Extract index of optimal parameter setting
    error_min_index = np.nanargmin(error_sum)

    # Return full error curve for that parameter setting
    return error[error_min_index]


# Functions to generate individual figures
def fig_learning_curves(data_delta, data_bayes, itrials_mark, ax):
    ...
    for t, c in zip(itrials_mark, colorlist(len(itrials_mark))):
        ...
        ax.annotate("",
                    ...
                    arrowprops=dict(arrowstyle="<|-", color=c, linewidth=2.5))
    ...
    cbar.ax.tick_params(labelsize=0.675 * rc['xtick.labelsize'])
...
def fig_lrate_curves(data_delta, data_bayes, itrials, ax):
    ...
    for y, e, c, ti in zip(logmed_delta, ebars_delta, colors, itrials):
        ax.errorbar(data_delta['lrates'], y, yerr=e,
                    linestyle='-', linewidth=3, color=c,
                    capsize=5.0, elinewidth=2,
                    marker='o', mfc='white', markersize=6, markeredgewidth=2,
    ...
    for y, e, c in zip(logmed_bayes, ebars_bayes, colors):
        ...
        hline = ax.plot(xlims, [y, y], color=c, linestyle='--')
    ...
    # Legend
    leg = ax.legend(loc=3,
                    bbox_to_anchor=(1.0, 0.25),  # on top: (.05, .9); to the side: (1.0, 0.25)
                    labelspacing=0.25)
...
if __name__ == '__main__':
    ...
    summary_fn = 'median'  # measure of central tendency to plot ('median' or 'mean')
    ...
    grid = GridSpec(1, 24, figure=fig, wspace=2., left=0, right=0.95, bottom=0.2, top=0.7)

    # Show image of RNN architecture
    ax = fig.add_subplot(grid[:, :8])
    ax.imshow(plt.imread('./rnn_util/rnn.png'))
    ax.axis('off')
    panel_label('a', 0, ax)

    # Plot error curves for Bayesian vs. classical delta rule
    ax = fig.add_subplot(grid[:, 9:15])
    fig_learning_curves(data_delta_red, data_bayes, itrials_compare, ax)
    panel_label('b', -0.35, ax)

    # Plot error curves for Bayesian vs. classical delta rule
    ax = fig.add_subplot(grid[:, 18:])
    fig_lrate_curves(data_delta, data_bayes, itrials_compare, ax)
    panel_label('c', -0.35, ax)

    # Save
    fig.savefig('./figs/rnn_figs.pdf', bbox_inches='tight')
